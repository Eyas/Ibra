...
namespace Ibra.Logging
{
    public struct LogWriter
    {
        ...
        internal LogWriter(Source source, Level level, string context, int line, SourceRoutes routes)
        {
            _prelude = $"{DateTime.Now,24} {source.Name,15}:{line,3} {level.ToString(),10} {context,30}  ";
            _level = level;
            _routes = routes;
        }
